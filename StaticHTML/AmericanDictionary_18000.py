...
for idx in range(18000, 35001):
    ...
    if idx % 1000 == 0:
        ...
        df_temp_result.to_csv(f'temp_dictionary_18000_to_{idx}.csv', index=False)

    # 關閉瀏覽器
driver.quit()
df_result = pd.DataFrame(list(df_dict.items()), columns=['title', 'content'])
df_result.to_csv('final_dictionary_18000.csv', index=False)
